...
export default function HUD() {
  ...
  return (
    <div
      ...
      className={`z-50 relative flex h-[90px] md:h-[120px] border-4 border-slate-200 rounded-full outline-4 outline-slate-400 transition-all duration-500 bg-gradient-to-br from-slate-200 to-slate-400 drop-shadow-lg ${
        toggleItemBar ? "w-max" : "w-[90px] md:w-[120px]"
      }`}
    >
      {/* Main Button */}
      <button
      ...
      >
        ...
        <div>
          <span className="text-center bg-white drop-shadow-lg px-2 py-1 md:px-4 md:py-2 rounded-lg absolute opacity-0 group-hover:opacity-100 -top-0 w-full left-0 group-hover:-top-6 md:group-hover:-top-8 transition-all">
            Your Bag
          </span>
        </div>
      </button>
      ...
      {toggleItemBar && (
        <ul className="items-center flex gap-2 pl-4 pr-6 md:pr-8">
          {[
